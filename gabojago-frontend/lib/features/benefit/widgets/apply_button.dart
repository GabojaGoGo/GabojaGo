...
class _ApplyButtonState extends State<ApplyButton> with WidgetsBindingObserver {
  bool _loading = false;
  bool _applied = false;
  int _applyCount = 0;
  ...
  void initState() {
    ...
    if (_isRepeatable) {
      _applyCount = UserDataService.instance.getBenefitApplyCount(
        widget.benefit.id,
      );
    } else {
      _applied = UserDataService.instance.isBenefitApplied(widget.benefit.id);
    }
  }
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            _isRepeatable
                ? '${widget.benefit.title} $_applyCount번째 이용이 기록됐어요 ✓'
