...
| 영역 | 명령 | 실패 시 의미 |
| --- | --- | --- |
| Backend | `./gradlew check --no-daemon` | 컴파일 또는 테스트 실패 |
...
현재 analyzer 정보·warning은 `tools/frontend-analyzer-baseline.txt`에 기준선으로 관리한다. 새 진단은 CI를 실패시키며, 기존 진단을 정리한 경우에만 `make frontend-analyze-baseline`으로 기준선을 갱신한다.
...
2. 새 서비스 로직은 정상 경로와 실패/폴백 경로를 최소 한 개의 테스트로 검증한다.
...
- Java 자동 포맷터와 Dart strict analyzer 옵션은 기존 코드 정리를 별도 PR로 진행한 뒤 추가한다.
