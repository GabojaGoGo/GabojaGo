...
처음 환경을 구성하거나 Docker·iOS 소셜 로그인·OSRM까지 함께 실행할 때는 [로컬 환경 설정과 전체 실행](local-development.md)을 먼저 따른다.
...
- 소셜 계정 하나(`provider + providerUserId`)는 가보자GO 내부 계정 하나에만 연결한다.
- 이메일은 프로필·연락 정보일 뿐 로그인 식별자나 계정 통합 기준으로 사용하지 않는다. 같은 이메일의 다른 소셜 계정은 별도 계정으로 생성한다.
- 현재 계정 연동·자동 병합·별도 본인 인증은 제공하지 않는다. 인증된 소셜 계정으로만 해당 계정의 데이터에 접근할 수 있다.
