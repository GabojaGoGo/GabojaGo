...
1. 미확정 다음 슬롯 1~2개까지 Beam Search look-ahead를 적용해 후보 점수에 반영
2. 출발지·여행지를 부산 고정값에서 좌표·지역 선택으로 확장
3. 프론트에서 후보 선택 즉시 핀·폴리라인을 누적 렌더링
4. 선택한 슬롯 상태를 임시 저장하거나 사용자 코스로 영속화
5. OSRM 장애 시 재시도·관측 지표·명확한 사용자 안내 강화
