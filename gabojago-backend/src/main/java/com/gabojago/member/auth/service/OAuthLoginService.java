...
public class OAuthLoginService {
  ...
  public OAuthLoginResponse login(OAuthProvider provider, String accessToken, String nonce) {
    OAuth2UserInfo userInfo = getUserInfo(provider, accessToken, nonce);
  ...
  public OAuthLoginResponse login(OAuth2UserInfo userInfo) {
    try {
      ...
      log.warn(
          "동시 가입 충돌 감지, 재시도: provider={}, providerUserId={}",
          userInfo.provider(),
  ...
  OAuthLoginResponse processLogin(OAuth2UserInfo userInfo) {
    ...
    if (socialAccount != null && !socialAccount.getUser().isActive()) {
      log.info(
          "탈퇴 사용자 소셜 연결 정리: socialAccountId={}, userId={}",
          socialAccount.getId(),
  ...
  private OAuthLoginResponse issueTokens(User user, boolean isNewUser) {
    ...
    try {
      ...
      return new OAuthLoginResponse(
          accessToken, refreshToken, userId, user.getNickname(), isNewUser);
  ...
  public OAuth2UserInfo getUserInfo(OAuthProvider provider, String accessToken, String nonce) {
    return switch (provider) {
      case KAKAO -> kakaoClient.getUserInfo(accessToken, nonce);
      case NAVER -> naverClient.getUserInfo(accessToken, nonce);
      case GOOGLE -> googleClient.getUserInfo(accessToken, nonce);
      case APPLE -> appleClient.getUserInfo(accessToken, nonce);
    };
  }
  ...
  public void unlink(OAuthProvider provider, String providerUserId) {
    switch (provider) {
      ...
      case APPLE -> appleClient.unlink(providerUserId);
    }
  }
}
