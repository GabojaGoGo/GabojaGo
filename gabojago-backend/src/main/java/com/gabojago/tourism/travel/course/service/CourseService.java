...
public class CourseService {
  ...
  private static final Map<String, RegionCenter> REGION_CENTERS =
      Map.ofEntries(
  ...
  public List<CourseDto> getRecommendedCourses(
      List<String> purposes, String duration, Double lat, Double lng, String travelConcept) {
    ...
    return List.of(TravelMode.CAR, TravelMode.WALK).stream()
        .flatMap(
            travelMode ->
                recommendedCourses(regionKey, normalizedDuration, travelConcept, travelMode)
                    .stream())
        .toList();
  }

  private List<CourseDto> recommendedCourses(
      String regionKey, String duration, String travelConcept, TravelMode travelMode) {
  ...
  /** 신규 DB 기반 추천 코스는 목록 응답에 places를 포함한다. 상세 API는 기존 화면 호환용으로 빈 값을 반환한다. */
  public CourseDetailDto getCourseDetail(String contentId, List<String> purposes) {
    return new CourseDetailDto(
        contentId, "", "", "DB_RECOMMENDATION", List.of(), List.of(), List.of());
  }
  ...
  private CourseDto toCourseDto(
    ...
    return new CourseDto(
        "recommendation_"
            + summary.travelMode().name().toLowerCase(Locale.ROOT)
            + "_"
            + route.rank(),
        title,
        summary.regionName(),
  ...
  private String buildOverview(RouteRecommendationResponse.RouteCandidate route) {
    ...
    String warningText =
        route.warnings().isEmpty() ? "주의 경고 없음" : String.join(", ", route.warnings());
    return "Beam Search 기반 DB 추천 루트입니다. "
        + "총점 "
        + route.totalScore()
        + "점"
        + " · 장소점수 "
        + breakdown.getOrDefault("placePreference", 0.0)
        + " · 이동페널티 "
        + breakdown.getOrDefault("movementPenalty", 0.0)
        + " · 시간적합 "
        + breakdown.getOrDefault("timeFit", 0.0)
        + " · 균형보정 "
        + breakdown.getOrDefault("routeBalance", 0.0)
        + " · "
        + warningText;
  }
    ...
    return REGION_CENTERS.entrySet().stream()
        .min(
            java.util.Comparator.comparingDouble(
                entry -> squaredDistance(lat, lng, entry.getValue().lat(), entry.getValue().lng())))
        .map(Map.Entry::getKey)
        .orElse(DEFAULT_REGION_KEY);
  }

  private static double squaredDistance(
      double lat, double lng, double targetLat, double targetLng) {
