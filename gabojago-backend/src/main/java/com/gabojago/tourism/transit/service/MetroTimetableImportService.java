...
public class MetroTimetableImportService {
  ...
  public ImportResult replaceFrom(Reader source) throws IOException {
    ...
    List<MetroTrainStopTime> values =
        timetable.stops().stream()
            .map(
                stop ->
                    MetroTrainStopTime.of(
                        stop.sourceDate(),
                        stop.dayType(),
                        stop.lineNumber(),
                        stop.trainNumber(),
                        stop.terminalName(),
                        result.stationCodes().get(key(stop.lineNumber(), stop.stationName())),
                        stop.stopSequence(),
                        stop.arrivalSeconds(),
  ...
  private ImportResult mapped(MetroTimetableCsvParser.Timetable timetable) {
    Map<String, String> stationCodes =
        stationRepository.findAll().stream()
            .collect(
                Collectors.toMap(
                    station -> key(station.getLineNumber(), station.getName()),
                    MetroStation::getStationCode,
                    (left, right) -> left));
    if (stationCodes.isEmpty())
      throw new IllegalStateException("시간표를 적재하기 전에 정적 도시철도 그래프를 적재해야 합니다.");
    List<String> missing =
        timetable.stops().stream()
            ...
            .toList();
    if (!missing.isEmpty())
      throw new IllegalArgumentException("시간표 역을 그래프에 연결하지 못했습니다: " + String.join(", ", missing));
    return new ImportResult(
        timetable.stops().size(),
        timetable.stops().stream()
            .map(MetroTimetableCsvParser.RawStopTime::trainNumber)
            .distinct()
            .count(),
        Map.copyOf(stationCodes));
  }
  ...
  public record ImportResult(
      int stopTimeCount, long trainCount, Map<String, String> stationCodes) {}
}
