...
- [ ] 빌드 오류 없이 실행되는가
- [ ] 새 HTTP endpoint가 있으면 OpenAPI 설명·예시·오류 응답과 서비스 테스트(정상/실패 또는 fallback)를 추가했는가
- [ ] 화면 사용자 흐름을 바꾸면 위젯 테스트를 추가·갱신했는가. 외부 API·지도·OSRM 흐름이면 iOS 시뮬레이터 수동 검증 또는 `integration_test` 결과를 PR에 남겼는가
